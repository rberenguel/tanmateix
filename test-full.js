...
for (let i = 0; i < Math.min(3, q1.premises.length); i++) {
  ...
  const dir = p.properties.direction === 1 ? '>' : (p.properties.direction === -1 ? '<' : '=');
  console.log(`    ${p.entities[0].displayValue} ${dir} ${p.entities[1].displayValue} (${p.properties.dimension})`);
}
const c = q1.conclusion;
const cDir = c.properties.direction === 1 ? '>' : (c.properties.direction === -1 ? '<' : '=');
console.log(`  Conclusion: ${c.entities[0].displayValue} ${cDir} ${c.entities[1].displayValue} (${c.properties.dimension})`);
...
for (let i = 0; i < mq.questions.length; i++) {
  ...
  console.log(`  Q${i + 1}: ${q.conclusion.type.name} conclusion, valid=${q.isValid}, strategy=${q.metadata.conclusionStrategy}`);
}
...
console.log(`✓ Generated categorical question with ${q4.premises.length} premises`);
console.log(`  Premises sample:`);
for (let i = 0; i < Math.min(3, q4.premises.length); i++) {
  const p = q4.premises[i];
  const rel = p.properties.same ? 'same as' : 'different from';
  console.log(`    ${p.entities[0].displayValue} ${rel} ${p.entities[1].displayValue}`);
}
const c4 = q4.conclusion;
const rel4 = c4.properties.same ? 'same as' : 'different from';
console.log(`  Conclusion: ${c4.entities[0].displayValue} ${rel4} ${c4.entities[1].displayValue}`);
...
console.log(`✓ Generated ultra-mixed question with ${q5.premises.length} premises`);
